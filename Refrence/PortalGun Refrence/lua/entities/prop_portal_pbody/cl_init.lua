...
local portal_1_color = CreateClientConVar("portal_color_PBODY_1", 2, true)
local portal_1_contraste = CreateClientConVar("portal_color_PBODY_contraste_1", 1, true)
local portal_1_saturation = CreateClientConVar("portal_color_PBODY_saturation_1", 0, true)

local portal_2_color = CreateClientConVar("portal_color_PBODY_2", 0, true)
local portal_2_contraste = CreateClientConVar("portal_color_PBODY_contraste_2", 1, true)
local portal_2_saturation = CreateClientConVar("portal_color_PBODY_saturation_2", 0, true)
...
function ENT:Think()
        ...
        local portaltype = self:GetNWInt("Potal:PortalType",TYPE_BLUE_PBODY)
...
if GetConVarNumber("portal_color_PBODY_1") >=14 then
		...
		elseif GetConVarNumber("portal_color_PBODY_1") >=11 then
			...
			if GetConVarNumber("portal_color_PBODY_saturation_1") >=2 then
			glowcolor = Color(171,117,145,255)
			elseif GetConVarNumber("portal_color_PBODY_saturation_1") >=1 then
...
if GetConVarNumber("portal_color_PBODY_contraste_1") >=2 then	
brightness = 7
elseif GetConVarNumber("portal_color_PBODY_contraste_1") >=1 then
brightness = 5
else
brightness = 3
end	

        if portaltype == TYPE_ORANGE_PBODY then
if GetConVarNumber("portal_color_PBODY_2") >=14 then
		...
		elseif GetConVarNumber("portal_color_PBODY_2") >=11 then
			...
			if GetConVarNumber("portal_color_PBODY_saturation_2") >=2 then
			glowcolor = Color(171,117,145,255)
			elseif GetConVarNumber("portal_color_PBODY_saturation_2") >=1 then
			glowcolor = Color(199,89,146,255)
			else		
			glowcolor = Color(255,32,150,255)
		end
...
if GetConVarNumber("portal_color_PBODY_contraste_2") >=2 then	
brightness = 7
elseif GetConVarNumber("portal_color_PBODY_contraste_2") >=1 then
brightness = 5
else
brightness = 3
end	
...
two_color_gray2:SetFloat("$PortalStatic", 1)
                        ...
                        if portaltype == TYPE_BLUE_PBODY then
						if GetConVarNumber("portal_color_PBODY_1") >=14 then
                        ...
                        elseif portaltype == TYPE_ORANGE_PBODY then
	...
	if IsInFront( pos, self:GetRenderOrigin(), self:GetForward() ) and self:GetNWBool("Potal:Activated",false) then
		...
		render.SetStencilEnable( true )
			...
			cam.Start3D2D(self:GetRenderOrigin(),self:GetAngles(),1)
				...
				local ToRT = portaltype == TYPE_BLUE_PBODY and texFSB or texFSB2
...
hook.Add("PostDrawOpaqueRenderables","DrawPortals PBODY", function()
	for k,v in pairs(ents.FindByClass("prop_portal_pbody"))do
		v:DrawPortal()
	end
end)
...
hook.Add( "ShouldDrawLocalPlayer PBODY", "Portal.ShouldDrawLocalPlayer PBODY", function()
...
hook.Add( 'PostDrawEffects PBODY', 'PortalSimulation_PlayerRenderFix PBODY', function()
	cam.Start3D( EyePos(), EyeAngles() )
	cam.End3D()
end)

hook.Add( "RenderScene PBODY", "Portal.RenderScene PBODY", function( Origin, Angles )
	// render each portal
				if GetConVarNumber("portal_texFSB") >=2 then
...
hook.Add( "HUDPaint PBODY", "Portal.BlueMonitor PBODY", function( w,h )
	if GetConVarNumber("portal_debugmonitor") == 1 and GetConVarNumber("sv_cheats") == 1 then
		// render each portal
		for k, v in ipairs( ents.FindByClass( "prop_portal_pbody" ) ) do
		  // debug monitor
			if view and v:GetNWInt("Potal:PortalType", TYPE_BLUE_PBODY) == TYPE_BLUE_PBODY then
...
/*------------------------------------
        GetMotionBlurValues()
------------------------------------*/
hook.Add( "GetMotionBlurValues PBODY", "Portal.GetMotionBlurValues PBODY", function( x, y, fwd, spin )
        if RENDERING_PORTAL then
                return 0, 0, 0, 0
        end
end )

hook.Add( "PostProcessPermitted PBODY", "Portal.PostProcessPermitted PBODY", function( element )
...
hook.Add( "RenderScreenspaceEffects PBODY", "Portal.RenderScreenspaceEffects PBODY", function()
...
hook.Add("Think PBODY", "Reset Camera Roll PBODY", function()
