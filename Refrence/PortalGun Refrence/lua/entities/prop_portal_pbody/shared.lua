...
elseif GetConVarNumber("portal_color_PBODY_1") >=1 then
	...
	elseif int == TYPE_ORANGE_PBODY then
...
function ENT:GetOpposite() --Don't think this is being used..? Gets the portal type that it would need to be linked too
	if self.PortalType == TYPE_BLUE_PBODY then
		return TYPE_ORANGE_PBODY
	elseif self.PortalType == TYPE_ORANGE_PBODY then
		return TYPE_BLUE_PBODY
	end
end
...
local function PlayerPickup( ply, ent )	
	if ent:GetClass() == "prop_portal_pbody" or ent:GetModel() == "models/blackops/portal_sides.mdl" then
		-- print("No Pickup.")
		return false
	end
	if ent:GetClass() == "prop_portal_pbody" or ent:GetModel() == "models/blackops/portal_sides_new.mdl" then
		-- print("No Pickup.")
		return false
	end
end
hook.Add( "PhysgunPickup PBODY", "NoPickupPortalssingleplayer PBODY", PlayerPickup )
hook.Add( "GravGunPickupAllowed PBODY", "NoPickupPortalssingleplayer PBODY", PlayerPickup )
hook.Add( "GravGunPunt PBODY", "NoPickupPortalssingleplayer PBODY", PlayerPickup )