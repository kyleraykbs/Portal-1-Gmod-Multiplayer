...
ENT.Linked = nil
ENT.PortalType = TYPE_BLUE_PBODY
ENT.Activated = false
ENT.KeyValues = {}
...
local vel_roof_max = CreateConVar("portal_velocity_roof", 9, {FCVAR_ARCHIVE,FCVAR_REPLICATED,FCVAR_SERVER_CAN_EXECUTE})
...
function ENT:SpawnFunction( ply, tr ) --unused.
	...
	local ent = ents.Create( "prop_portal_pbody" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:CleanMeUp()
	...
	if self.PortalType == TYPE_BLUE_PBODY then
...
elseif GetConVarNumber("portal_color_PBODY_1") >=1 then
	...
	ParticleEffect("portal_2_close_pbody",pos,ang,self)
		...
		elseif self.PortalType == TYPE_ORANGE_PBODY then
...
local function BulletHook(ent,bullet)
	...
	for k,inport in pairs(ents.FindByClass("prop_portal_pbody")) do --fix fake portal positions.
	...
	for i=1, bullet.Num do
		...
		if IsValid(tr.Entity) and tr.Entity:GetClass() == "prop_portal_pbody" then
	...
	for k,inport in pairs(ents.FindByClass("prop_portal_pbody")) do
		if inport:OnFloor() then
			inport:SetPos(inport:GetPos()+Vector(0,0,20))
		end
	end
end
hook.Add("EntityFireBullets", "BulletPorting PBODY", BulletHook)
...
function ENT:FindOpenPair() --This is for singeplayer, it finds a portal that is of the same type.
	local portals = ents.FindByClass( "prop_portal_pbody" );
...
hook.Add("SetupPlayerVisibility PBODY", "Add portalPVS PBODY", function(ply,ve)
	for k,self in pairs(ents.FindByClass("prop_portal_pbody"))do
		...
		-- if self:GetNWInt("Potal:PortalType") == TYPE_ORANGE_PBODY then
