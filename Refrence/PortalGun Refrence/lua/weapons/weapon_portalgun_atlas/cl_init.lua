...
hook.Add("HUDPaint ATLAS", "View model pickup override ATLAS", function(vm)
...
function SWEP:ViewModelDrawn(vm)
	...
	for k,v in pairs(VWhiteSprites) do
		...
		local drawpos = pos + ang:Forward() * v.pos.x + ang:Right() * v.pos.y + ang:Up() * v.pos.z
			...
			if last == TYPE_BLUE_ATLAS then
					col = Color(255, 255, 255, 255)	
			elseif last == TYPE_ORANGE_ATLAS then
					col = Color(255, 255, 255, 255)
	end
...
function SWEP:DrawHUD()
	...
	if hEnt != nil && hEnt:IsValid() && hEnt:GetClass() == "prop_portal_atlas" then
		if hEnt:GetNWInt("Potal:PortalType",TYPE_BLUE_ATLAS) == TYPE_BLUE_ATLAS then
			validRed = false
		else
			validBlu = false
		end
...
local lastPort = self:GetNetworkedInt("LastPortal",0)	
	...
	if (lastPort == TYPE_BLUE_ATLAS) then
		lastPort_left = "["
	else
		lastPort_left = "{"
	end
	
	if (lastPort == TYPE_ORANGE_ATLAS) then
		lastPort_right = "]"
	else
		lastPort_right = "}"
	end
...
if !system:GetBool() then 	
	...
	elseif lastPort == TYPE_ORANGE_ATLAS then
		if GetConVarNumber("portal_color_ATLAS_2") >=14 then
			surface.SetDrawColor(c_Gray2)
		elseif GetConVarNumber("portal_color_ATLAS_2") >=13 then
			surface.SetDrawColor(c_Gray)
...
killicon.Add( "prop_portal_atlas", "hud/killicon_portals", Color( 255, 48, 0, 255 ) )
