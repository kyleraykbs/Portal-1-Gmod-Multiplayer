...
if ( CLIENT ) then
		SWEP.PrintName          = "Portal Gun Atlas's"
...
if SERVER then
	...
	hook.Add( 'AllowPlayerPickup ATLAS', 'PortalPickup ATLAS', function( ply, ent )
		if IsValid( ply:GetActiveWeapon() ) and IsValid( ent ) and ply:GetActiveWeapon():GetClass() == 'weapon_portalgun_ATLAS' then --and (table.HasValue( pickable, ent:GetModel() ) or table.HasValue( pickable, ent:GetClass() )) then
			return false
		end
	end )
end

hook.Add("Think ATLAS", "Portalgun Holding Item ATLAS", function()
...
RunConsoleCommand("portal_texFSB", "1")
end
...
function SWEP:ShootBall(type,startpos,endpos,dir)
	local ball = ents.Create("projectile_portal_ball_atlas")
...
function SWEP:ShootPortal( type )
	...
	local OrangePortalEnt_ATLAS = owner:GetNWEntity( "Portal:Orange_ATLAS", nil )
	local BluePortalEnt_ATLAS = owner:GetNWEntity( "Portal:Blue_ATLAS", nil )
   
	local EntToUse = type == TYPE_BLUE_ATLAS and BluePortalEnt_ATLAS or OrangePortalEnt_ATLAS
	local OtherEnt = type == TYPE_BLUE_ATLAS and OrangePortalEnt_ATLAS or BluePortalEnt_ATLAS
					  ...
					  timer.Simple( hitDelay_Instant, function()
							if ball and ball:IsValid() then 
								...
								if !IsValid( EntToUse ) then
										...
										local Portal = ents.Create( "prop_portal_atlas" )
										...
										if type == TYPE_BLUE_ATLAS then
												...
												owner:SetNWEntity( "Portal:Blue_ATLAS", Portal )
												Portal:SetNetworkedBool("blue_ATLAS",true,true)
											   
										else
									   
												owner:SetNWEntity( "Portal:Orange_ATLAS", Portal )
												Portal:SetNetworkedBool("blue_ATLAS",false,true)
...
function SWEP:PrimaryAttack()
        ...
        self:ShootPortal( TYPE_BLUE_ATLAS )
...
function SWEP:SecondaryAttack()
        ...
        self:ShootPortal( TYPE_ORANGE_ATLAS )
...
function SWEP:CleanPortals()
        ...
        local blueportal_ATLAS = self.Owner:GetNWEntity( "Portal:Blue_ATLAS" )
        local orangeportal_ATLAS = self.Owner:GetNWEntity( "Portal:Orange_ATLAS" )
        local cleaned = false
       
        for k,v in ipairs( ents.FindByClass( "prop_portal_atlas" ) ) do
       
                if v == blueportal_ATLAS or v == orangeportal_ATLAS and v.CleanMeUp then
...
function SWEP:CheckExisting()
	if blueportal_ATLAS != nil && blueportal_ATLAS != nil then return end
	for _,v in pairs(ents.FindByClass("prop_portal_atlas")) do
		local own = v.Ownr
		if v != nil && own == self.Owner then
			if v.type == TYPE_BLUE_ATLAS && self.blueportal_ATLAS == nil then
				self.blueportal_ATLAS = v
			elseif v.type == TYPE_ORANGE_ATLAS && self.orangeportal_ATLAS == nil then
				self.orangeportal_ATLAS = v
			end
		end
	end
end
